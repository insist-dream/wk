<template>
  <div class="self">
    ...
    <div class="cf-sort">
      <div @click="Default_sort" class="hover">默认排序</div>
      <div>价格
        <p class="arrows">
      ...
      <div class="classify-list">
        <div v-for="(item,index) in classify" class="classify-li">
          <div class="cd-list-img">
            ...
            <span :class="zizhu_arr[index]" ><span class="test">{{ item.special_tags.title }}</span></span>
            <div></div>
          </div>
          <div class="cd-list-text">
            <p class="cd-list-title"><router-link :to="{name:'self_driving',params:{id:item.product_id}}" tag="a">{{ item.product_name }}</router-link></p>
            <p class="cd-list-type">
              <span>{{item.type_name}}</span>
              <span>{{item.city_name}}出发</span>
              <span>{{item.day_num}}天{{item.night_num}}晚</span>
            </p>
            <p class="cd-list-description advantage">{{ item.advantage }}</p>
...
//          console.log(arr)
    ...
    sortPrice_max:function(){                       //按照价格排序 ----max
      ...
      for(let i = 0; i < this.classify.length; i++){
        this.zizhu_arr[i] = {tags:true,tags_1:false,tags_2:false,tags_3:false,tags_4:false,tags_5:false,tags_6:false,tags_7:false,tags_8:false}
    ...
    get_type:function(){
      ...
      let params ={
        appid: '5101',
        sign: md5("appid=5101&nonce=" + nonce + "&params={\"channel_code\":\"SELFHELP\",\"page\":1,\"page_size\":1}&timestamp=" + timestamp + "&b3b01c69cc8408593f708514a211b09f"),
        nonce: nonce,
        timestamp: timestamp,
        params:{
  ...
  get_list:function(){                     //获取列表
    ...
    let nonce = onlyid(32, 52);   //  生成唯一ID
    let timestamp = times();   // 生成时间戳
    let pap = "{\"max_price\":"+ this.max_price +",\"min_price\":"+ this.min_price +",\"max_day_num\":"+ this.max_day_num +",\"min_day_num\":"+ this.min_day_num +",\"themes\":["+ this.themes +"],\"destinations\": ["+this.destinations+"],\"channel_code\":\"SELFHELP\",\"page\":"+this.page_num+",\"page_size\":20,\"image_size\":\"L\"}"
    let params ={
      appid: '5101',
//      sign: md5("appid=5101&nonce=" + nonce + "&params={\"max_price\":"+ this.max_price +",\"min_price\":"+ this.min_price +",\"max_day_num\":"+ this.max_day_num +",\"min_day_num\":"+ this.min_day_num +",\"themes\":["+ this.themes +"],\"destinations\": ["+this.destinations+"],\"channel_code\":\"LUXURY\",\"page\":1,\"page_size\":20}&timestamp=" + timestamp + "&b3b01c69cc8408593f708514a211b09f"),
    ...
    this.$http.post(get_list_api,
      ...
      for(let i = 0; i < self.classify.length; i++){//class="tags"
        self.zizhu_arr[i] = {tags:true,tags_1:false,tags_2:false,tags_3:false,tags_4:false,tags_5:false,tags_6:false,tags_7:false,tags_8:false}
        self.zizhu_arr.push({tags:true,tags_1:false,tags_2:false,tags_3:false,tags_4:false,tags_5:false,tags_6:false,tags_7:false,tags_8:false})
      ...
      Default_sort:function(){
        ...
        let nonce = onlyid(32, 52);   //  生成唯一ID
        let timestamp = times();   // 生成时间戳
        let pap = "{\"max_price\":"+ this.max_price +",\"min_price\":"+ this.min_price +",\"max_day_num\":"+ this.max_day_num +",\"min_day_num\":"+ this.min_day_num +",\"themes\":["+ this.themes +"],\"destinations\": ["+this.destinations+"],\"channel_code\":\"SELFHELP\",\"sort\":\"4\",\"page\":1,\"page_size\":"+this.number+",\"image_size\":\"L\"}"
        let params ={
          appid: '5101',
          sign: md5("appid=5101&nonce=" + nonce + "&params="+pap+"&timestamp=" + timestamp + "&b3b01c69cc8408593f708514a211b09f"),
          nonce: nonce,
