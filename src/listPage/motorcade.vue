      ...
      <div class="classify-list">
        <div v-for="(item,index) in classify" class="classify-li">
          <div class="cd-list-img">
            ...
            <span :class="chedui_arr[index]" ><span class="test">{{ item.special_tags.title }}</span></span>
            <div></div>
          </div>
...
<script>
  import {homeList, get_list_api, order_add_api} from '../data/api/api'
...
//          console.log(arr)
    ...
    sortPrice_max:function(){                       //按照价格排序 ----max
      ...
      for(let i = 0; i < this.classify.length; i++){
        this.chedui_arr[i]={tags:true,tags_1:false,tags_2:false,tags_3:false,tags_4:false,tags_5:false,tags_6:false,tags_7:false,tags_8:false}
    ...
    get_type:function(){
      ...
      let params ={
        appid: '5101',
        sign: md5("appid=5101&nonce=" + nonce + "&params={\"channel_code\":\"MOTORCADE\",\"page\":1,\"page_size\":1}&timestamp=" + timestamp + "&b3b01c69cc8408593f708514a211b09f"),
        nonce: nonce,
        timestamp: timestamp,
        params:{
  ...
  get_list:function(){
    ...
    let nonce = onlyid(32, 52);   //  生成唯一ID
    let timestamp = times();   // 生成时间戳
    let pap = "{\"max_price\":"+ this.max_price +",\"min_price\":"+ this.min_price +",\"max_day_num\":"+ this.max_day_num +",\"min_day_num\":"+ this.min_day_num +",\"themes\":["+ this.themes +"],\"destinations\": ["+this.destinations+"],\"channel_code\":\"MOTORCADE\",\"page\":1,\"page_size\":20,\"image_size\":\"L\"}"
    let params ={
      appid: '5101',
...
//    this.$http.post('http://api.wktrip.com/product/get_list',
    this.$http.post(get_list_api,
      qs.stringify(params)
        ...
        for(let i = 0; i < self.classify.length; i++){
          self.chedui_arr[i]={tags:true,tags_1:false,tags_2:false,tags_3:false,tags_4:false,tags_5:false,tags_6:false,tags_7:false,tags_8:false}
          self.chedui_arr.push({tags:true,tags_1:false,tags_2:false,tags_3:false,tags_4:false,tags_5:false,tags_6:false,tags_7:false,tags_8:false})
  ...
  get_more_list:function(){                //加载更多列表
    ...
    let nonce = onlyid(32, 52);   //  生成唯一ID
    let timestamp = times();   // 生成时间戳
    let pap = "{\"max_price\":"+ this.max_price +",\"min_price\":"+ this.min_price +",\"max_day_num\":"+ this.max_day_num +",\"min_day_num\":"+ this.min_day_num +",\"themes\":["+ this.themes +"],\"destinations\": ["+this.destinations+"],\"channel_code\":\"MOTORCADE\",\"page\":"+this.page_num+",\"page_size\":20,\"image_size\":\"L\"}"
    let params ={
      appid: '5101',
      sign: md5("appid=5101&nonce=" + nonce + "&params="+pap+"&timestamp=" + timestamp + "&b3b01c69cc8408593f708514a211b09f"),
      nonce: nonce,
...
//    this.$http.post('http://api.wktrip.com/product/get_list',
    this.$http.post(get_list_api,
      qs.stringify(params)
        ...
        for(let i = 0; i < self.classify.length; i++){//class="tags"
          ...
          tag = self.getclassbycolor(self.classify[i].special_tags.color,self.classify[i].special_tags.title)
          console.log(self.classify[i].special_tags.color);
          if(tag){
            self.chedui_arr[i][tag] = true
          }
        }
  ...
  Default_sort:function(){                    //默认排序
    this.number = this.page_num*20
    let nonce = onlyid(32, 52);   //  生成唯一ID
    let timestamp = times();   // 生成时间戳
    let pap = "{\"max_price\":"+ this.max_price +",\"min_price\":"+ this.min_price +",\"max_day_num\":"+ this.max_day_num +",\"min_day_num\":"+ this.min_day_num +",\"themes\":["+ this.themes +"],\"destinations\": ["+this.destinations+"],\"channel_code\":\"MOTORCADE\",\"sort\":\"4\",\"page\":1,\"page_size\":"+this.number+",\"image_size\":\"L\"}"
    let params ={
      appid: '5101',
      sign: md5("appid=5101&nonce=" + nonce + "&params="+pap+"&timestamp=" + timestamp + "&b3b01c69cc8408593f708514a211b09f"),
      nonce: nonce,
